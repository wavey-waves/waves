    ...
    try {
      if (joinType === "anonymous") {
        ...
        try {
          ...
          const { id, userName, color, token } = loginRes.data;
          // Store token if provided (for desktop app)
          if (token) {
          ...
          if (loginErr.response?.status === 400 && loginErr.response?.data?.message === "Invalid credentials") {
            try {
              ...
              const { _id, userName, color, token } = signupRes.data;
              // Store token if provided (for desktop app)
              if (token) {
                localStorage.setItem('authToken', token);
              }
        ...
        if (isCreating) {
          ...
          const { _id, userName, color, token } = response.data;
          // Store token if provided (for desktop app)
          if (token) {
            localStorage.setItem('authToken', token);
          }
          ...
          const { id, userName, color, token } = response.data;
          // Store token if provided (for desktop app)
          if (token) {
            localStorage.setItem('authToken', token);
          }
